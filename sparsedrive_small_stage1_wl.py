...
load_from = 'ckpt/Stage1_LiDAR_only_25epoch_iter_175800.pth'
resume_from = None
...
model = dict(
    ...
    camera_grad_scale=1 / 40, # カメラバックボーンの勾配をスケーリング
    img_backbone=dict(
        type="ResNet",
        depth=50,
        num_stages=4,
    ...
    pts_backbone=dict(  
        type="VoxelNet",  
        in_channels=5,  # x, y, z + 反射強度 + タイムスタンプ  
        voxel_size=(0.2, 0.2, 0.4),  # ボクセルサイズ (x, y, z) 
        point_cloud_range=(-30, -60, -5, 30, 60, 3),  # 点群の範囲  roi_size=(30, 60)から設定
        max_num_points=32,  # 各ボクセル内の最大ポイント数  
        max_voxels=(32000, 80000),  # (train, test)の最大ボクセル数  
        vfe_channels=(128, 256),  # Voxel Feature Encodingのチャネル数  
        middle_channels=(128, 256, 512, 1024),  # 中間層のチャネル数  
    ), 
    pts_neck=dict(  
        type="PtsFPN",  
    ...
    fusion_module=dict(  
        ...
        img_channels=embed_dims,  # 256  
        pts_channels=embed_dims,  # 256  
        out_channels=embed_dims,  # 256  
        num_levels=num_levels,  # 4  
        fusion_method="bidirectional_attention",  # "concat", "add", "attention", "bidirectional_attention"のいずれか  
        # concat方式の場合、チャネル数を調整  
        use_channel_reduction=False,  # concatの場合はTrue  
        # bidirectional_attention方式の場合以下の設定を追加
        use_self_attn=True,  # セルフアテンションを有効化
    ),  
    # ==========================================================================================='''
...
train_pipeline = [
    ...
    dict(  # ← 追加
        type="PointSample",  
        num_points=34000,  # NuScenesの平均的なポイント数  
        sample_method='random',  
    ),  
    dict(type="WrapPointsAsDataContainer", cpu_only=False),  # ← 追加
...
optimizer = dict(
    type="AdamW",
    #lr=3e-4,
    lr=4e-4, # batch_size = 4
    weight_decay=0.001,
    paramwise_cfg=dict(
        custom_keys={
            "img_backbone": dict(lr_mult=0.5),
            "pts_vfe": dict(lr_mult=0.1),
            "pts_middle": dict(lr_mult=0.1),
            "pts_backbone": dict(lr_mult=0.1),
            "pts_projection": dict(lr_mult=0.1),
            "pts_neck": dict(lr_mult=0.1),
            "fusion_module": dict(lr_mult=0.1),
        }
    ),
)
...
#custom_hooks = [  
#    dict(  # 統計量の表示
#        type='ModuleGradientNormHook',  
#        log_interval=51, #51,  
#        log_detailed_stats=True  # 詳細統計を有効化  
#    ),  
#    dict(  # 統計量の表示
#        type='ModuleOutputStatsHook',  
#        log_interval=51, #51  
#    ),
#]
